...
export default function App() {
  ...
  const handleAddReport = async (location, status, description) => {
    ...
    if (!coordinates) {
      alert(
        'Location not found. Try: "Mumbai", "Delhi", "Kakinada", "Bengaluru", "Chennai"'
      );
      return false;
    }
    const newReport = {
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <p style={{ fontSize: '20px', color: '#4B5563', lineHeight: '1.8' }}>
            Real-time garbage bin monitoring powered by AI. Optimize collection
            routes, reduce overflow, and track environmental impact—all in one
            platform.
          </p>
  ...
  // ─── AUTH FORM (shared by signup + login) ────────────────────────
  const AuthForm = ({ mode }) => {
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              onClick={() =>
                setCurrentPage(mode === 'signup' ? 'login' : 'signup')
              }
              style={{
                backgroundColor: 'transparent',
                border: 'none',
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '32px' }}>
            {['list', 'map'].map((mode) => (
              <button
                ...
                style={{
                  backgroundColor: viewMode === mode ? '#10B981' : '#E5E7EB',
